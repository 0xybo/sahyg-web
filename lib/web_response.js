class WebResponse {
	...
	renderError(name) {
		let { code } =
			this.Web.config.get(["status", name.toUpperCase()]) ||
			((name = "SERVER_ERROR"),
			{
				code: 500,
			});

		this.render("errors/" + name.toUpperCase(), {}, code);
	}
	error(name, { details, description }) {
		if (details) this.details = details;
		if (message) this.description = description;
	...
	send() {
		this.res.status(200/*this.statusCode*/).send({
