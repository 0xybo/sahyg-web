class Page {
	...
	setGet(path, ...callback) {
		...
		path = path.filter(
			function (p) {
				return (
					roots.find((root) => root.paths.includes(p) && root.type == "GET" && root.enabled) ||
					(this.logger.warn(`Path GET:${p} is disabled`), false)
				);
			}.bind(this)
		);
	...
	setPost(path, ...callback) {
		...
		path = path.filter(
			function (p) {
				return (
					roots.find((root) => root.paths.includes(p) && root.type == "POST" && root.enabled) ||
					(this.logger.warn(`Path POST:${p} is disabled`), false)
				);
			}.bind(this)
		);
