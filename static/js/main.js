...
SAHYG.off = function (typeOrID, elementOrCallback) {
	if (typeof typeOrID == "number") {
		...
		SAHYG.Events[typeOrID] = SAHYG.Events[typeOrID].filter((event) => event.element != elementOrCallback && event.callback != elementOrCallback);
};
...
SAHYG.Components = {
				...
				SAHYG.createElement(
					...
					SAHYG.createElement(
						...
						data.group?.name == "administrator"
							...
							: null,
						data.group?.name == "vip" ? SAHYG.createElement("span", { class: "lafr", style: "color: var(--blue-600)" }, "&#xf005;") : null
					)
				)
			).get(0).outerHTML;
		},
		menu(target, items, options = {}) {
						...
						if (item.type == "divider") menu.append(SAHYG.createElement("span", { class: "divider", ...(item.attributes || {}) }));
							...
							menu.append(
								SAHYG.createElement(
									"div",
									{
										...
										on: {
											click: function () {
												if (!item.keepOpen) instance.hide();
							...
							menu.append(
								SAHYG.createElement(
									...
									SAHYG.createElement("span", { class: "icon lafs" }, item.icon || ""),
									SAHYG.createElement("span", { class: "text" }, item.text),
									(dropdownTarget = SAHYG.createElement("span", { class: "dropdown-icon" }, "&#xf105;"))
								)
							);
							SAHYG.Components.tooltipMenu(dropdownTarget, item.dropdown, { trigger: "mouseenter focus" });
						}
					}
	...
	loader: {
		...
		replaceContent(element, loadingText = true) {
			element = $(element);
			...
			popup
				...
				.content(
					SAHYG.createElement(
						...
						SAHYG.createElement("a", { href: "/about#cookies", on: { click: popup.close } }, await SAHYG.translate("MORE_INFORMATIONS"))
					)
				)
				.button("ok", {
							...
							inputs.map(async (input) => {
								...
								if (["text", "url"].includes(input.type))
									inputElement = SAHYG.createElement("input", {
										events: {
											change: (event) => {
												$(event.target).closest("[data-input-type]").removeClass("invalid");
									...
									inputElement = await SAHYG.Components.input.select({
										...
										events: {
											input: ({ target }) => {
												$(target).closest("[data-input-type]").removeClass("invalid");
												data[input.name] = $(target).attr("data-value");
											},
										},
										placeholder: input.placeholder,
									});
									...
									inputElement = await SAHYG.Components.input.boolean({
										...
										events: {
											input: ({ target }) => {
												$(target).closest("[data-input-type]").removeClass("invalid");
												data[input.name] = $(target).attr("value") == "true";
											},
										},
									});
									...
									inputElement = SAHYG.createElement("input", {
										type: "color",
										on: {
											input: ({ target }) => $(target).attr("value", (data[input.name] = $(target).val())),
										},
										value: input.defaultValue,
									});
