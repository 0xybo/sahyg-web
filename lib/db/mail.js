function applyFunctions(model) {
	model.send = function () {
		if (model.content?.type == "html") {
			...
			return this.Web.mail.send({ content: model.content?.text, target: model.target, subject: model.subject, locale: model.locale });
		}
	}.bind(this);
}
